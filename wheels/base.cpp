...
void IgnWheel::init()
{
    ...
    if (spkmode == 4) {
        ...
        if (ram4.adv_offset < -200) {
            tmp_offset = -200;
        } else if (ram4.adv_offset > 200) {
            tmp_offset = 200;
        } else {
    ...
    /* ----------------------  420A/NEON MODE ------------------------ */
        ...
        if (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL)) {      //if COP mode or SEQ then double up pattern
            flagbyte5 |= FLAGBYTE5_CAM;
            deg_per_tooth[8] = 200;
            deg_per_tooth[9] = 200;
        ...
        if (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL)){      //if COP mode or use cam then double up pattern
            cycle_deg = 7200;
            flagbyte5 |= FLAGBYTE5_CAM;
...
//            trigger_teeth[1] = 6;
//        } else {
            // non RX8 mode 180 deg different
            trigger_teeth[0] = 6;
            trigger_teeth[1] = 20;
//        }
        ...
        if (((ram4.EngStroke & 0x03) != 0x03)
            && (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL))) {
      ...
      com_cas:
        ...
        if (ram4.spk_config & 2) { // cam speed twin trigger
            cycle_deg = 7200;
            if (ram4.no_cyl != 2) {
                conf_err = 29;
            }
            ...
            if ((ram4.no_cyl & 0x1f) == 2) {
                if (ram4.sequential & (SEQ_FULL | SEQ_SEMI)) {
                    conf_err = 150;
                }
        ...
        deg_per_tooth[4] = 1300;

        smallest_tooth_crk = 510;
        smallest_tooth_cam = 0;
        ...
        if (num_cyl == 4) {
            ...
            if ((ram4.spk_mode3 & 0xc0) || (ram4.sequential & (SEQ_SEMI | SEQ_FULL))) {      //if W/S or COP mode or use cam then double up pattern
                cycle_deg = 7200;
                flagbyte5 |= FLAGBYTE5_CAM;
        ...
        if (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL)){      // COP or use-cam
            cycle_deg = 7200;
            flagbyte5 |= FLAGBYTE5_CAM;
            deg_per_tooth[32] = 100;
            deg_per_tooth[33] = 100;
        ...
        if (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL)) {      //if COP mode or use cam then double up pattern
            ...
            cycle_deg = 7200;
            /* unpleasant cam pattern requires triggering on both edges and tooth size measurement */
            no_teeth = 14;           // COP / seq
            deg_per_tooth[7] = 600;
            deg_per_tooth[8] = 600;
            deg_per_tooth[9] = 600;
        ...
        trigger_teeth[2] = 7;
        trigger_teeth[3] = 10;
        trig_angs[0] = -830 + tmp_offset;
        trig_angs[1] = -830 + tmp_offset;
        trig_angs[2] = -830 + tmp_offset;
        trig_angs[3] = -830 + tmp_offset;
        ...
        last_tooth = no_teeth;
        if ((ram4.ICIgnOption & 0x8) || ((ram4.no_cyl & 0x1f) != 8)) {
            conf_err = 52; // even V8 required
        } 
        ...
        if (((ram4.spk_mode3 & 0xc0) == 0x80) || (ram4.sequential & SEQ_FULL)) {
...
/* ----------------------  Jeep 2000 & 2002 -------------------- */
       ...
       if (ram4.no_cyl == 6) {
        ...
        if ((ram4.No_Teeth == 135) && (ram4.no_cyl == 5)) {
            no_teeth = 90;
            no_triggers = 5;
            teethpt = 18;
            degpt = 80;
            fly_div = 3;
        } else if ((ram4.No_Teeth == 135) && (ram4.no_cyl == 6)) {
            no_teeth = 90;
            no_triggers = 6;
            teethpt = 15;
            degpt = 80;
            fly_div = 3;
        } else if ((ram4.No_Teeth == 136) && (ram4.no_cyl == 8)) {
            no_teeth = 136;
            no_triggers = 8;
            teethpt = 17;
            degpt = 53; // actually 5.294 deg/tooth
            fly_div = 2;
        } else if ((ram4.No_Teeth == 132) && (ram4.no_cyl == 4)) {
            no_teeth = 88;
            no_triggers = 4;
            teethpt = 22;
            degpt = 82; // actually 8.18
            fly_div = 3;
        } else if ((ram4.No_Teeth == 130) && (ram4.no_cyl == 4)) {
            no_teeth = 52;
            no_triggers = 4;
            teethpt = 13;
        ...
        /* 36-2 on the crank with a 3 on the cam at 120deg spacing*/
        unsigned int i;
        cycle_deg = 7200;
        flagbyte5 |= FLAGBYTE5_CAM;
        no_triggers = 6;
...
//    } else if (spkmode == 50) {
    ...
    if (ram4.feature3 & 0x08) {
        tooth_init = trigger_teeth[ram4.trig_init - 1];
    }
}

void IgnWheel::calc_absangs()
{
